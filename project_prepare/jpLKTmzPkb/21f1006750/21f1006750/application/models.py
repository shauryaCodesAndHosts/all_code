...
class Product(db.Model):
    ...
    def serialize(self):
        return {
            ...
            'manufacturingDate': self.manufacturingDate,
            'ratePerUnit': self.ratePerUnit,
            'categoryId': self.categoryId,
            'inStock': self.inStock,
            'timeOfEntry': self.timeOfEntry
        }
...
class CustomerOrders(db.Model):
    ...
    orderTotal = db.Column(db.Integer,nullable = False)
    numberOfItems = db.Column(db.Integer, nullable= False)
    customerId= db.Column(db.Integer, db.ForeignKey('customers.customerId'))
    listOfItems = db.Column(db.String, nullable = False)
    quantityOfItems = db.Column(db.String)
    customer = db.relationship('Customers',backref=db.backref('customerOrders', lazy=True))


class SoldProductData(db.Model): 
    __tablename__ = "soldProductsData"
    soldProductsId = db.Column(db.Integer, primary_key = True, nullable = False, autoincrement= True)
    soldProductName = db.Column(db.String)
    soldProductFreq = db.Column(db.Integer)
    timeOfSelling = db.Column(db.String)
    def serialize(self):
        return {
            'Product_Name': self.soldProductName
        }